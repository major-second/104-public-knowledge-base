...
- 过拟合：把训练样本中的特点当成了所有潜在样本都有的性质，导致测试性能下降
  ...
  - 哲学上：P不等于NP，过拟合就不可避免，否则通过[[reduction]]就$P=NP$了
- 模型选择：如何让泛化误差小？
  - 当然：现实中还有时空复杂性、可解释性等
# 评估方法
- 测试集
  - [[iid]]于训练集
    - 实际中有时使用有部分[[domain-gap]]的，将就一下
  - 不能在训练过程中使用过
    - 故：典型错误：[[cross-validation]]但迭代承前训练
- [[cross-validation]]
- 留出：直接划分成两个互斥集合
  - 如（针对样本比例）分层采样
  - 多次这样留出可以得到多个评估结果，还能算分布等
  - 最终结果如何得出
    - 典型：用留出法，先找到好的参数，然后用整个集合再训
    - 实际中[[ensemble#bagging]]往往更好用（当然计算开销大）
- 留出的[[tradeoff]]
  - 如果训练集S太大，评估不稳定准确
  - 如果S太小，则保真性fidelity不强（不能反映全集D上模型的性能）
  - [[bias-variance-tradeoff]]角度：S小评估结果方差大，否则偏差大
  - 一般而言，测试集至少30个样例，常见做法使用1/5到1/3
- 自助[[bootstrap-in-stats]]
  - 数据集小好用
  - 数据集大时没必要（改变分布了）
- 调参：如设[[grid-search]]
  - 有指数爆炸问题，如3个参数，每个参数5档，就125种了
  - 可以认为调参和选择算法没有本质区别
    - 例如：[[MLP]]“网络层数”为1时退化为[[OLS]]
# 性能度量
...
- 对二分类问题，有“正”“负”，地位不对等，能考察更精细的度量
  - precision（查准率、准确率）
    ...
    - 极端情况100%
      ...
      - 若“正例”对应“否定假设”，则对应[[power-level]]中水平为0（然而并不能直接类比，参考[[power-level]]）
    - 所以GT中如果1太少（不均衡），可能准确率就一直很低
      - 均衡数据才适宜于这个
  - recall（查全率、召回率）
    - 所有真的中有多少是正例
      - 即：实际上正的集合中多少能被检测出来
    - 直接对应[[power-level]]的功效
    - 极端情况100%
